...
export default function UsedItemDetail(props) {
   ...
   useEffect(() => {
      setPickedItems(JSON.parse(localStorage.getItem("pickedItems") || "[]"));
   }, []);

   useEffect(() => {
      if (
         pickedItems.filter((el) => el._id === data?.fetchUseditem._id)
            .length === 1
      ) {
         setIsPicked(true);
      } else {
   ...
   const onClickPickItem = async () => {
      const usedItemData = data.fetchUseditem;
      const pickedItems = JSON.parse(
         localStorage.getItem("pickedItems") || "[]"
      );
      const temp = pickedItems.filter(
         (pieckedEl) => pieckedEl._id === usedItemData._id
      );
      if (temp.length === 1) {
         // 찜하기 취소
         const temp = pickedItems.filter(
         ...
         localStorage.setItem("pickedItems", JSON.stringify(temp));
      }
      if (temp.length !== 1) {
         // 찜하기 등록
         const pickedObj = { _id: usedItemData._id, name: usedItemData.name };
         pickedItems.push(pickedObj);
         localStorage.setItem("pickedItems", JSON.stringify(pickedItems));
      }
      try {
         const result = await pickUseditem({
            variables: { useditemId: router.query.number },
         });
         console.log(result);
